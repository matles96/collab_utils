...
def run(round, run_al):
    ...
    train_command = '%s tools/train.py '%PYTHON + \
    ...
    eval_command = '%s tools/test.py '%PYTHON + \
               ' %s ' % cfg.get('train_config') + \
    ...
    unlabeled_infer_command = '%s tools/test.py '%PYTHON + \
    ...
    if run_al:
        ...
        diversity_infer_command = '%s tools/test.py ' % PYTHON + \
